    ...
    if predict_button:
        with st.spinner('🔄 Calculating yield prediction...'):
            ...
            try:
                ...
                # Clamp yield to range 1.0 to 9.0 tons per hectare
                yield_value = np.clip(prediction[0], 1.0, 9.0)
                yield_value = round(yield_value, 2)  # Round to 2 decimal places
                
                # Calculate R2 score (from your training - you'll need to replace this with actual value)
                r2_score = 0.91  # Your actual R2 score from model evaluation
                
                # Display result - only yield and R2 score
                st.markdown(f"""
                    <div class="yield-display">
                        <div class="yield-value">{yield_value}</div>
                        <div class="yield-unit">tons per hectare</div>
                        <div style="margin-top: 20px; font-size: 1.2em; color: #555;">
                            <b>R² Score:</b> {r2_score:.4f}
                        </div>
                    </div>
                """, unsafe_allow_html=True)
        ...
        The model will calculate the expected yield in **tons per hectare** (range: 1.0 - 9.0).
        """)
...
st.markdown("""
    <div style='text-align: center; color: white;'>
        ...
        <p>Yield predictions range from <b>1.0 to 9.0 tons per hectare</b>.</p>
